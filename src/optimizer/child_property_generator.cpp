    ...
    switch (prop->Type()) {
      ...
      case PropertyType::COLUMNS: {
        ...
        for (size_t col_idx = 0; col_idx < col_len; col_idx++) {
          ...
          expression::ExpressionUtil::GetAggregateAndTvExprs(
              aggr_exprs, tv_exprs, expr.get());
...
void ChildPropertyGenerator::JoinHelper(const BaseOperatorNode *op) {
  ...
  if (!has_star) {
    // All base columns and agg expr in join condition needs to be generated by
    // the child
    std::vector<std::shared_ptr<expression::AggregateExpression>> agg_exprs;
    std::vector<std::shared_ptr<expression::TupleValueExpression>> tv_exprs;
    expression::ExpressionUtil::GetAggregateAndTvExprs(agg_exprs, tv_exprs,
                                                       join_cond);
    for (const auto &expr : tv_exprs) child_cols.insert(expr);
    for (const auto &expr : agg_exprs) child_cols.insert(expr);
    ...
    for (const auto &child_col : child_cols) {
      ...
      if (util::IsSubset(left_table_alias, table_alias_set)) {
        ...
        PL_ASSERT(util::IsSubset(right_table_alias, table_alias_set));
